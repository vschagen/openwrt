...
static void eip93_hash_complete_request(void *context, u32 pe_ctrl_stat, u32 pe_length)
{
        struct crypto_async_request *async = context;
...
again:
	scoped_guard(spinlock_irqsave, &eip93->ring->write_lock) {
		...
		if (unlikely(eip93_desc_ring_full(cdr_ring))) {
                        eip93_kick_engine(eip93, 0, 0);
        		usleep_range(EIP93_RING_BUSY_DELAY,
			     EIP93_RING_BUSY_DELAY * 2);
        		goto again;
        	}
                if (last) {
                      idx = eip93_set_callback(eip93, eip93_hash_complete_request, async);
